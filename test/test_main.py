...
class TestPydanticModels:
    ...
    def test_login_response_structure(self):
        ...
        response = LoginResponse(access_token="token123", token_type="bearer", expires_in=3600)
...
class TestLoginEndpoint:
    ...
    def test_login_with_valid_api_key(self, client, valid_api_key):
        ...
        response = client.post("/auth/login", json={"api_key": valid_api_key})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_login_with_invalid_api_key(self, client, invalid_api_key):
        ...
        response = client.post("/auth/login", json={"api_key": invalid_api_key})
    ...
    def test_multiple_logins_return_different_tokens(self, client, valid_api_key):
        ...
        response1 = client.post("/auth/login", json={"api_key": valid_api_key})
        response2 = client.post("/auth/login", json={"api_key": valid_api_key})
...
class TestDevOpsEndpoint:
    ...
    def test_devops_post_with_valid_credentials(self, client, headers_with_valid_credentials):
        ...
        response = client.post("/DevOps", json=payload, headers=headers_with_valid_credentials)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_devops_missing_api_key(self, client, headers_missing_api_key):
        ...
        response = client.post("/DevOps", json=payload, headers=headers_missing_api_key)
        assert response.status_code == 403
    ...
    def test_devops_invalid_api_key(self, client, headers_invalid_api_key):
        ...
        response = client.post("/DevOps", json=payload, headers=headers_invalid_api_key)
        assert response.status_code == 403
    ...
    def test_devops_missing_jwt_token(self, client, headers_missing_jwt):
        ...
        response = client.post("/DevOps", json=payload, headers=headers_missing_jwt)
    ...
    def test_devops_expired_jwt_token(self, client, headers_expired_jwt):
        ...
        response = client.post("/DevOps", json=payload, headers=headers_expired_jwt)
        assert response.status_code == 401

    @pytest.mark.skip(
        reason="Token reuse detection flaky in parallel test environment - replay_store persists across tests"
    )
    def test_devops_token_reuse_prevention(self, client, headers_with_valid_credentials):
        ...
        response1 = client.post("/DevOps", json=payload, headers=headers_with_valid_credentials)
        assert response1.status_code == 200

        # Segundo request con el mismo token debe ser rechazado
        response2 = client.post("/DevOps", json=payload, headers=headers_with_valid_credentials)
    ...
    def test_devops_invalid_json_payload(self, client, headers_with_valid_credentials):
        ...
        response = client.post("/DevOps", content="not json", headers=headers)
...
class TestIntegration:
    ...
    def test_complete_flow_login_and_devops(self, client, valid_api_key):
        ...
        # Paso 1: Login
        login_response = client.post("/auth/login", json={"api_key": valid_api_key})
        ...
        devops_response = client.post(
            "/DevOps",
            json=payload,
            headers={"X-Parse-REST-API-Key": valid_api_key, "X-JWT-KWY": token},
        )
    ...
    def test_multiple_requests_with_different_tokens(self, client, valid_api_key):
        ...
        login1 = client.post("/auth/login", json={"api_key": valid_api_key})
        token1 = login1.json()["access_token"]

        response1 = client.post(
            "/DevOps",
            json=payload,
            headers={"X-Parse-REST-API-Key": valid_api_key, "X-JWT-KWY": token1},
        )
        assert response1.status_code == 200

        # Segundo request con token 2
        login2 = client.post("/auth/login", json={"api_key": valid_api_key})
        token2 = login2.json()["access_token"]

        response2 = client.post(
            "/DevOps",
            json=payload,
            headers={"X-Parse-REST-API-Key": valid_api_key, "X-JWT-KWY": token2},
        )
